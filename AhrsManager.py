...
class AhrsManager():
    ...
    def update(self):
        ...
        for datum in microstrainData:
            ...
            if 'scaledAccelX' in datum.keys():
                self.accX = datum['scaledAccelX']*9.81
            if 'scaledAccelY' in datum.keys():
                self.accY = datum['scaledAccelY']*9.81
            if 'scaledAccelZ' in datum.keys():
                self.accZ = datum['scaledAccelZ']*9.81
    ...
    with AhrsManager(csv_file_name="test_ahrs.csv") as am:
        class looper():
            ...
            def __call__(self):
                ...
                self.i+=1
        GracefulDeathLoop().non_blocking_loop(looper() , dt=1.0/100.) 
