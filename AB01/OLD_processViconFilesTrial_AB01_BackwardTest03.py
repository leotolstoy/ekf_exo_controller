...
if __name__ == '__main__':
	...
	filename = 'AB01 EKF Reverse Test Protocol V2, Exp 03.csv'

	AB_SUBJECT = 'AB01'
	# Load in forceplate data

	n_skiprows_forceplate_1 = 969# row num that contains RightForcePlates:
	...
	nrows = 30003*10 #=  row index containing the final forceplate data row
	...
	# Load in HS data
	nrows_HS = 965 #row number of the last HS row
	...
	# MARKER
	n_skiprows_marker_1 = 301005# row num that contains AB0's:
	...
	nrows = 30003 #=  num frames, alsolast idx - row with frame subframe
	...
	speedTimes = np.array(list(range(0,310,10)))
	print(len(speedTimes))
	speeds = np.array([1, 1.2, 0.8, 1.2, 0.8, \
	...
	df.to_csv(f'Vicon_{AB_SUBJECT}BackwardTest03_processed.csv',index=False)
