...
if __name__ == '__main__':
	...
	filename = 'AB01 FootMarkers EKFTPV4A 07.csv'

	AB_SUBJECT = 'AB01'
	# Load in forceplate data

	n_skiprows_forceplate_1 = 267# row num that contains RightForcePlates:
	n_skiprows_forceplate_2 = n_skiprows_forceplate_1+1#contains Frame, subframe
	n_skiprows_forceplate_3 = n_skiprows_forceplate_1-1#row before marker names

	nrows = 8002*10 #=  row index containing the final forceplate data row

	skiprows_forceplate = [i for i in range(n_skiprows_forceplate_1)]
	skiprows_forceplate.append(n_skiprows_forceplate_2)

	df_forceplate = pd.read_csv(filename,skiprows=skiprows_forceplate,nrows=nrows)
	# print(df_forceplate.head())
	# print(df_forceplate.tail())
	col_rename_forceplate = {
		'Fx':'RightForcePlate.Fx',
		'Fy':'RightForcePlate.Fy',
		'Fz':'RightForcePlate.Fz',
		'Mx':'RightForcePlate.Mx',
		'My':'RightForcePlate.My',
		'Mz':'RightForcePlate.Mz',
		'Cx':'RightForcePlate.Cx',
		'Cy':'RightForcePlate.Cy',
		'Cz':'RightForcePlate.Cz',
		'Fx.1':'LeftForcePlate.Fx',
		'Fy.1':'LeftForcePlate.Fy',
		'Fz.1':'LeftForcePlate.Fz',
		'Mx.1':'LeftForcePlate.Mx',
		'My.1':'LeftForcePlate.My',
		'Mz.1':'LeftForcePlate.Mz',
		'Cx.1':'LeftForcePlate.Cx',
		'Cy.1':'LeftForcePlate.Cy',
		'Cz.1':'LeftForcePlate.Cz',

		}

	# print(df_forceplate['Fx'].head())
	df_forceplate.rename(columns=col_rename_forceplate,inplace=True)
	# print(df_forceplate['RightForcePlate.Fx'].head())
	print(df_forceplate.head())
	print(df_forceplate.tail())


	# Plot forceplate stuff
	fig, axs = plt.subplots(3,1,sharex=True,figsize=(10,6))
	frames_forceplate = df_forceplate['Frame'].to_numpy()
	print(len(frames_forceplate))
	time_forceplate = np.linspace(frames_forceplate[0] - frames_forceplate[0],frames_forceplate[-1] - frames_forceplate[0],len(frames_forceplate))/100

	Fx = df_forceplate['RightForcePlate.Fx'].to_numpy()
	Fy = df_forceplate['RightForcePlate.Fy'].to_numpy()
	Fz = -df_forceplate['RightForcePlate.Fz'].to_numpy()
	axs[0].plot(time_forceplate, Fx, label='Fx')
	axs[1].plot(time_forceplate, Fy, label='Fy')
	axs[2].plot(time_forceplate, Fz, label='Fz')
	...
	# Load in HS data
	nrows_HS = 263 #row number of the last HS row
	...
	# MARKER
	n_skiprows_marker_1 = 80293# row num that contains AB0's:
	...
	nrows = 8002 #=  num frames, alsolast idx - row with frame subframe
	...
	timeToAccel = (1.2 - 0.8)/0.5

	speedTimes = np.array(list(range(0,80,20)))
	print(len(speedTimes))
	speeds = np.array([1.2,0.8,1.2,0.8])
	print(len(speeds))
	...
	for i, HS in enumerate(HSs):
		...
		if i == 0:
			...
			# speed = 1
			HS_time_adj = np.floor(HS_time)
			print(f'HS_time_adj: {HS_time_adj}')
			speedIdx = np.argmin(np.abs(HS_time_adj - speedTimes))
			speedIdx = int(HS_time//20)
			print(f'speedIdx: {speedIdx}')
			speed = speeds[speedIdx]
			speeds_from_vicon[prevHS_idx:HS_idx+1] = speed
			# strideLength = (np.abs(RHEE_X[HS_idx] - np.min(RHEE_X[prevHS_idx:HS_idx+1]))/1000)
			strideLength = (RHEE_Y[HS_idx] - RHEE_Y[prevHS_idx])/1000 + (speed*strideTime)
			# strideLength = (speed*strideTime)
			...
			incline_from_vicon[prevHS_idx:HS_idx+1] = 0
	...
	df.to_csv(f'Vicon_{AB_SUBJECT}TPV4A_07_processed.csv',index=False)
