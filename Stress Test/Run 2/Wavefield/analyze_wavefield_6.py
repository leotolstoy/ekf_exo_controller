...
def main():
    ...
    df = pd.read_csv('wavefield6_HS.csv')
    #7.31
    ...
    data = np.loadtxt("Wave_6.csv", delimiter=',')

    TIME_START = 19.2 #calculated from video, using full time vector
    
    timeSec_full=data[:,0]
    startIdx = np.argmin(np.abs(TIME_START - timeSec_full))
    print(f'startIdx: {startIdx}')
    
    VIDEO_TIME_START = 617/60
    VIDEO_TIME_END = 4069/60 #IN THE VIDEO
    TRIAL_DURATION = VIDEO_TIME_END - VIDEO_TIME_START #CALCULATED FROM VIDEO
    TIME_END = TRIAL_DURATION+TIME_START #TOTAL TRIAL DURATION EKF, USING FULL TIME VECTOR
    ...
    for i,x in enumerate(data[:]):
        ...
        if vid_HS_times[0] <= timeSec <= vid_HS_times[-1]:
            ...
            if HSDetected_ground_truth and i > 0:
                ...
                print(oldLen)
                print(f'phase_stride_error_sq: {phase_stride_error_sq}')
    ...
    phase_rmse_TBE_stdev = np.std(phase_stride_rms_TBE_data)
    phase_rmse_TBE_ground_truth_stdev = np.std(phase_stride_rms_TBE_ground_truth_data)

    print('TBE')
    print('phase rmse mean: {0}'.format(phase_rmse_TBE_mean))
    print('phase rmse stdev: {0}'.format(phase_rmse_TBE_stdev))

    print('TBE ground truth')
    print('phase rmse mean: {0}'.format(phase_rmse_TBE_ground_truth_mean))
    print('phase rmse stdev: {0}'.format(phase_rmse_TBE_ground_truth_stdev))
