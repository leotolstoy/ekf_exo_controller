...
if __name__ == '__main__':
	...
	filename = 'AB06_EKF_TPVA_01.csv'

	AB_SUBJECT = 'AB06'

	# Load in HS data
	nrows_HS = 273 #row number of the last HS row
	...
	# MARKER
	n_skiprows_marker_1 = 80903# row num that contains AB0's:
	...
	nrows = 8062 #=  num frames, alsolast idx - row with frame subframe
	...
	df = pd.DataFrame({'time': time_vicon,
				   ...
				   'RHEE_Y': RHEE_Y[firstIdx:prevHS_idx],
				   'RHEE_Z': RHEE_Z[firstIdx:prevHS_idx],
                   'HS_vicon': HSs
                   })
	df.to_csv(f'Vicon_{AB_SUBJECT}TPVA_01_processed.csv',index=False)
